...
def search_label(name):
    ...
    labels_path = "recyclables_origin/labels/"
    folder_list = os.listdir(labels_path)

    for folder in folder_list:
        folder_list = os.listdir(labels_path + folder)
...
def get_center_points(datas, scale, resize_name, output_path):
    label_num_list = []
    label_name_list = []
    center_points_list = []
    for data in datas:
        en_data = check_data(data["cls"], data["detail"])
        label_no = en_data["no"]
